...
public class DashboardService : IDashboardService
{
    ...
    {
        var pendingVerifications = await _dashboardRepo.GetPendingVerificationCountAsync();
        var activeJobs = await _dashboardRepo.GetActiveJobPostingCountAsync();
        var totalContracts = await _dashboardRepo.GetTotalContractsCountAsync();
        var pendingJobPostings = await _dashboardRepo.GetPendingJobPostingCountAsync();
        var pendingReports = await _dashboardRepo.GetPendingReportsCountAsync();
    ...
    {
        var totalUsers = await _dashboardRepo.GetTotalUsersCountAsync();
        var totalParents = await _dashboardRepo.GetUserCountByRoleAsync("Parent");
        var totalNannies = await _dashboardRepo.GetUserCountByRoleAsync("Nanny");
        var totalModerators = await _dashboardRepo.GetUserCountByRoleAsync("Moderator");
        var activeUsers = await _dashboardRepo.GetUserCountByStatusAsync((int)UserStatus.Active);
        var inactiveUsers = await _dashboardRepo.GetUserCountByStatusAsync((int)UserStatus.Inactive);
        var pendingUsers = await _dashboardRepo.GetUserCountByStatusAsync((int)UserStatus.Pending);
        var bannedUsers = await _dashboardRepo.GetUserCountByStatusAsync((int)UserStatus.Banned);
    ...
    private async Task<RevenueStatsDto> GetRevenueStatsAsync()
    {
        ...
        var recentTransactions = await _dashboardRepo.GetRecentTransactionsAsync(5);
        var monthlyRevenue = await _dashboardRepo.GetMonthlyRevenueAsync(12);
        var completedTransactions = await _dashboardRepo.GetCompletedTransactionEventsSinceAsync(start12Months);
    ...
    private async Task<SubscriptionStatsDto> GetSubscriptionStatsAsync()
    {
        ...
        var total = await _dashboardRepo.GetTotalSubscriptionsAsync();
        var active = await _dashboardRepo.GetActiveSubscriptionsAsync();
        var expired = await _dashboardRepo.GetExpiredSubscriptionsAsync();
        var monthlySubscriptions = await _dashboardRepo.GetMonthlySubscriptionsAsync(12);
        var createdSubscriptions = await _dashboardRepo.GetCreatedSubscriptionDatesSinceAsync(start12Months);
    ...
    private async Task<QueueRangeStatsDto> GetQueueRangeStatsAsync(DateTime startDateUtc)
    {
        ...
        var pendingVerificationDates = await _dashboardRepo.GetPendingVerificationDatesSinceAsync(startDateUtc);
        var pendingJobPostingDates = await _dashboardRepo.GetPendingJobPostingDatesSinceAsync(startDateUtc);
        var pendingReportDates = await _dashboardRepo.GetPendingReportDatesSinceAsync(startDateUtc);
    ...
    private async Task<ModerationResultsStatsDto> GetModerationResultsAsync()
    {
        ...
        var verificationItems = await _dashboardRepo.GetVerificationModerationEventsSinceAsync(start12Months);
        var jobItems = await _dashboardRepo.GetJobPostingModerationEventsSinceAsync(start12Months);
    ...
    private async Task<UserGrowthStatsDto> GetUserGrowthStatsAsync()
    {
        ...
        var createdAtList = await _dashboardRepo.GetUserCreatedDatesSinceAsync(start12Months);
