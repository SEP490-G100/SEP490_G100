...
public class GetTopJobsForNannyAsyncTests
{
    ...
    public GetTopJobsForNannyAsyncTests()
    {
        var b = CreateSut();
        _mockRepo = b.Repo;
    ...
    // -- TC4: topK gi?i h?n k?t qu?, s?p x?p theo FinalScore -------------
    [Fact]
    public async Task TopKOrdering()
    {
        ...
        Assert.Equal(jobHigh.JobId, result[0].JobId);
        Assert.Equal(jobMid.JobId, result[1].JobId);
    }
    ...
    public async Task WithLocation_DistanceScoreCalculated()
    {
        ...
        Assert.Single(result);
        Assert.True(result[0].DistanceKm > 1000);
        Assert.InRange(result[0].DistanceScore, 0.09, 0.11);
    }
}
