...
public class GetConversationsTests
{
    ...
    // Condition: user không có hội thoại.
    // Confirmation: danh sách rỗng, chỉ 1 lần gọi repo.
    [Fact]
    public async Task Empty_ReturnsNoItems()
    {
        ...
        Assert.Empty(list);
        _mockRepo.Verify(r => r.GetConversationsByUserIdAsync(me), Times.Once);
    }
    ...
    // Condition: tin mới nhất bị xóa (soft delete).
    ...
    public async Task LastMessageDeleted_ShowsPlaceholder()
    {
        ...
        Assert.Equal("[Tin nhan da bi xoa]", item.LastMessage);
    }

    // Condition: tên rỗng → dùng email.
    [Fact]
    ...
    // Condition: hội thoại bị chặn phía tôi.
    [Fact]
