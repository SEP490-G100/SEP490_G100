...
public class GetPublicProfileTests
{
    ...
    // Condition: target không tồn tại.
    // Confirmation: cùng ngoại lệ như BuildProfileDtoAsync.
    [Fact]
    ...
    // Condition: người xem chính là chủ hồ sơ.
    // Confirmation: không redact; giữ email/phone/địa chỉ/gia đình như hồ sơ cá nhân.
    [Fact]
    public async Task SelfView_ReturnsFullProfileUnchanged()
    {
        ...
        Assert.Equal("1 Đường A", dto.Address);
        Assert.Equal("Nhà có trẻ", dto.FamilyDescription);
        Assert.NotNull(dto.Children);
        Assert.Single(dto.Children!);
    }

    // Condition: xem hồ sơ phụ huynh khi không phải chính họ.
    // Confirmation: ẩn liên hệ, vị trí, gia đình và dữ liệu trẻ theo GetPublicProfileAsync.
    [Fact]
    ...
    // Condition: xem bảo mẫu; nhánh isTargetNanny không redact thêm.
    // Confirmation: vẫn còn email, phone, địa chỉ, tọa độ, DOB/age.
    [Fact]
    ...
    // Condition: target không phải parent cũng không phải nanny.
    // Confirmation: nhánh else — ẩn PII cơ bản, không đụng tới mô tả gia đình (nếu có).
    [Fact]
