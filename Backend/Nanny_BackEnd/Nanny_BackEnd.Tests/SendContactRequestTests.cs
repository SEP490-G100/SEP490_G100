...
/// </summary>
public class SendContactRequestTests
{
    ...
    // Condition: user không phải parent.
    // Confirmation: 400.
    [Fact]
    ...
    // Condition: cùng user (parent) với tài khoản nanny.
    // Confirmation: 400, không tự gửi cho mình.
    [Fact]
    ...
    // Condition: nội dung > 1000 ký tự (sau trim).
    // Confirmation: 400.
    [Fact]
    ...
    // Condition: đã có request pending (status=0) với cùng cặp parent–nanny.
    // Confirmation: 409.
    [Fact]
    ...
    // Condition: lần đầu, chưa có request.
    // Confirmation: 200, Add, SaveChanges, thông báo cho nanny, message thành công mặc định.
    [Fact]
    ...
    // Condition: request cũ đã xử lý (status != 0) → gửi lại.
    // Confirmation: 200, không Add, cập nhật bản ghi, message gửi lại.
    [Fact]
